...
def augmentation = false // default file
def export = true // default true 
def extract_um = 256 // default 256
def tile_px = 512    // default 512
def background_intensity = 240     // default 240 or 220
def targetDir = "D:/MUCBERN-CRC-DX-IMGS/BLOCKS(wholeslide)/" // insert absolute path, forward slashes, ends with slash
...
for (obj in annotations) {
    if (obj.isAnnotation()) {
        ...
        if (roi.getClass() == qupath.lib.roi.RectangleROI && roi.getBoundsWidth()<=(5*extract_um)) {
            ...
            if (median_px >  background_intensity) { 
                print("Tile has >50% brightness > background_intensity, discarding")
                continue
            }
